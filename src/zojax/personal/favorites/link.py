...
from interfaces import IPersonalLink
...
class URLProperty(object):
    ...
    def __get__(self, inst, klass):
        ...
        if not urlparse.urlsplit(value)[0]:
            return 'http://%s'% value
...
class PersonalLink(PersistentItem):
    interface.implements(IPersonalLink)

    url = URLProperty(IPersonalLink['url'])
